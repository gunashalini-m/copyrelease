...
function invoiceDocumentHtml(invoice, options = {}) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${escapeHtml(invoice.number)}</title>
    ...
    <div class="page-break terms"><h2>Terms and Conditions</h2>${(invoice.terms || []).map((term) => `<p>${escapeHtml(term)}</p>`).join('')}</div>
  </body></html>`;
}
...
function openPrintableInvoice(invoice) {
  const html = invoiceDocumentHtml(invoice, { embedFonts: false });
